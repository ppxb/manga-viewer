...
const nextPublishAt = computed(() => dayjs(props.info.comic.next_publish_at).format('M月D日'))
</script>
...
<template>
  <div>
    <div relative border border-base rounded-lg flex="~ col justify-center gap-2" p4 pt12 transition-all hover:cursor-pointer hover:bg-hover>
      <img
        :src="props.info.domain.icon"
        absolute top-4 left-4 w20
      >
      <div absolute right-2 top-0 text-right font-mono text-3em op10 pointer-events-none>
        <span mr1>#</span>
        <span font-bold>{{ props.info.comic.episodes.length > 1 ? "Col" : props.info.comic.episodes[0].sort_volume }}</span>
      </div>
      <div font-sans-jp flex="~ items-center gap-12">
        <img
          :src="props.info.comic.logo"
          w64 object-fill mb1
        >
        <div flex="~ col justify-center gap-1">
          <div font-bold>
            {{ props.info.comic.title }}
          </div>
          <div op75 text-sm mb2>
            {{ props.info.comic.author }}
          </div>
          <div font-bold text-tiny>
            分类
            <span font-sans-jp text-category ml2>
              {{ props.info.comic.category }}
            </span>
          </div>
          <div font-bold text-tiny>
            种类
            <span
              v-for="tag in props.info.comic.tags" :key="tag"
      ...
      <div flex="~ gap-3">
        <div font-sans-jp font-bold rounded-2xl text-tiny bg-card w-fit flex="~ items-center gap-2" px3 py1>
          <span text-orange-400>
            {{ props.info.comic.episodes.length > 1 ? "最新话" : "当前话" }}
          </span>
          <div w1px h12px bg-neutral-900:10 dark:bg-neutral-300:15 />
          <span mr1>{{ props.info.comic.episodes[0].volume }}</span>{{ props.info.comic.episodes[0].title }}
        </div>
        <div font-sans-jp font-bold rounded-2xl text-tiny bg-card w-fit flex="~ items-center gap-2" px3 py1>
